...
while True:
    ...
    try:
        ...
        outfile = '../Blend Mass Balance Run List PYTHON v0.4 (0.300 DCW).xlsx'
        varfile = 'FDI Tags (do not touch)/tank_tags_map.csv'

        # Assumptions/Utilities
        conv_od_to_gpL = 0.300
        T_ref_degC = 25.0
        P_ref_barg = 0.0
        ...
        comps = ['argon', 'carbon_dioxide', 'ethane', 'hydrogen', 'methane', 'nitrogen', 'oxygen', 'propane']
        ...
        variable_dict = dict.fromkeys(['BDO, mg/L', 'OD, raw', 'Feed Scale, kg', 'BLEND_F_in', 'BLEND_x_argon_in_raw', 'BLEND_x_carbon_dioxide_in_raw', 'BLEND_x_ethane_in_raw', 'BLEND_x_hydrogen_in_raw', 'BLEND_x_methane_in_raw', 'BLEND_x_nitrogen_in_raw', 'BLEND_x_oxygen_in_raw', 'BLEND_x_propane_in_raw', 
        ...
        results.at[run_id, 'DCW (g/L/OD)'] = conv_od_to_gpL
        ...
        ## Inlet gas
        results.at[run_id, 'Inlet Gas Rate, mol/hr'] = results.at[run_id, 'BLEND_F_in'] * (1/1000) * 60 * (P_ref_barg + 1.0) / (0.08314 * (273 + T_ref_degC))
        # Normalize inlet gas composition, calculate MW
        mass_spec_sum = 0.0
        for x in range(0, len(comps)):
            tag = 'BLEND_x_' + comps[x] + '_in_raw'
            mass_spec_sum += results.at[run_id, tag]
        MW_sum = 0.0
        for x in range(0, len(comps)):
            tag_raw = 'BLEND_x_' + comps[x] + '_in_raw'
            tag_norm = 'BLEND_x_'+ comps[x] +'_in_normalized'
        ...
        results.at[run_id, 'Effluent Gas Rate, mol/hr'] = results.at[run_id, 'Inlet Gas Rate, mol/hr'] * results.at[run_id, 'BLEND_x_argon_in_normalized'] / results.at[run_id, 'x_argon_out_normalized']
        ...
        if feed_rate_end == 0.0:
            ...
            results.at[run_id, 'Liquid Feed Rate, g/hr'] = assumed_liq_feed_rate
        else:
        ...
        delta_w_bm_gpL = (od_end - od_start) * conv_od_to_gpL
        ...
        results.at[run_id, 'w_BDO_liq_g/L'] = results.at[run_id, 'BDO, mg/L'] / 1000.0
        results.at[run_id, 'w_BM_liq_g/L'] = results.at[run_id, 'OD, raw'] * conv_od_to_gpL
        ...
        beginning_cols = ['Run_ID', 'Tank', 'Start Time, hours', 'End Time, hours', 'DCW (g/L/OD)', 'C Closure', 'H Closure', 'O Closure', 'N Closure']
        reorganized_df = results[[col for col in beginning_cols] + [col for col in results if col not in beginning_cols]]

        # Exception handling for PermissionError (ie. if someone has the output file open)
        try:
