...
while True:
    ...
    try:
        ...
        components = [{'name':'argon',          'molar_mass':40, 'C':0, 'H':0, 'O':0, 'N':0},
        ...
        # CHANGE BACK TO variable_dict
        # Initialize dict with keys only (values = None)
        variable_dict = dict.fromkeys(['BDO, mg/L', 'OD, raw', 'Feed Scale, kg', 
        ...
        results.at[idx, 'N2 Inlet Gas Rate, mol/hr'] * results.at[idx, 'N2_x_argon_in_normalized'])/ \
        ...
        delta_w_bm_gpL = (od_end - od_start) * OD_to_gDCW_perL
