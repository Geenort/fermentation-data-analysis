...
2) Assumption: biomass loaded effluent liquid has density of 1000 g/L
3) acetate not considered
'''
...
# Primary inputs
tank = int(input("Enter the tank number (1-14): "))
...
tank = 14
run_id = 'CST251'
start_time = float(200)
end_time = float(300)
'''

# Input and output file strings
infile = '../CST Data Files/run_set_data_' + run_id + '.csv'
outfile = '../Mass Balance Run List_PYTHON_(0.42DCW)_version_v0c.xlsx'
...
if (abs(bdo_end - bdo_start) / bdo_end) > 0.25 or (abs(od_end - od_start) / od_end) > 0.02:
    print('WARNING: This is not a steady state time interval!')
...
feed_rate_start = raw_data.at[t_start, variable_dict['Tank Scale, grams']] - raw_data.at[t_start_plus1, variable_dict['Tank Scale, grams']]
feed_rate_end = raw_data.at[t_end_minus1, variable_dict['Tank Scale, grams']] - raw_data.at[t_end, variable_dict['Tank Scale, grams']]

if abs(((feed_rate_end - feed_rate_start) / feed_rate_end)) >= 0.05:
